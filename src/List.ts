...
export default class List<T> implements IList<T> {
    ...
    private _defaultComparer: IComparer<T> = (x: T, y: T) => {
        if (x === y) return 0;
        if (x > y) return 1;
        return -1;
    };

    constructor(collection?: IterableIterator<T> | T[]) {
        //super();
        this._itemCount = 0;
        if (collection != null) {
    ...
    public removeAt(index: number): void {
        ...
        if (index !== this._itemCount - 1) { // Nothing to move if last element.
    ...
    public addRange(collection: IterableIterator<T>) {
        if (!collection) {
    ...
    public insertRange(index: number, collection: IterableIterator<T>) {
        if (!collection) {
            throw new Error("Argument Null");
        }
        if (index >= this._itemCount) {
    ...
    public removeAll(predicate: (item: T) => boolean): void {
    ...
    public binarySearch(item: T, index?: number, count?: number, comparer?: IComparer<T>): number {
        if (index === undefined) {
            index = 0;
        }
        if (count === undefined) {
